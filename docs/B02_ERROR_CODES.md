...
| 200 detail | `REPLY_QUALITY_BLOCKED` | FAILED | 否 | 回信经质量审查和最多一次重写后仍被阻止；不自动重试，也不回显候选正文、内部审查详情、provider body 或本机路径 |
| 200 detail | `PERSONA_NOT_READY` | FAILED | 否 | 已配置 provider 且启用 Persona v2 时，发布快照不是 `READY`；在修复本地发布包前不得自动重试，且不回显文件路径、解析详情或 readiness gaps |
