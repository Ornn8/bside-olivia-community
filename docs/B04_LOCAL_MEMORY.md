...
设置页不会给官方历史导入 POST 设置固定总时限；大量信件仍按上述逐封边界处理，并以同路径 GET 进度和各 provider 自身的单次超时判断是否卡住，避免总耗时随信件数增长时被客户端提前中止。Mem0 写入超时后的结果确认也受同一写入时限约束；时限内无法确定结果时返回 `MEM0_WRITE_UNCERTAIN` 并失败关闭，不会无限等待或重发不确定写入。
