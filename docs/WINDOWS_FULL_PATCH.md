...
客户端启动协议以整个隔离 profile 目录为边界：启动器在创建 `profile/` 前记录该目录是否已存在，只有此前不存在才视为全新 profile。客户端始终以安装目录下的 `app/` 为工作目录。若客户端第一次退出码为 `0x0E000003`（包括已有 profile），启动器会在同一后端生命周期内重启客户端一次，因此客户端最多启动两次；正常退出及任何其他退出码均不重试，第二次退出码原样透传。每次启动和退出分别记录不含路径的 `client_start` 与 `client_exit`，并以 `attempt` 标记第 1 或第 2 次，退出记录只附整数退出码；触发重试时另记 `client_retry`，全新 profile 的固定原因为 `known_fresh_profile_exit`，已有 profile 则为 `known_client_exit`。这些诊断不记录安装路径、profile 路径、环境变量或凭据。同一安装目录的稳定启动器只允许一个启动生命周期；首个 Olivia 尚在运行时重复点击会记录 `launch_already_running` 并立即结束第二个启动器，不会重复启动后端或客户端。不同安装目录互不阻塞，进程异常退出后自动释放该启动占用。若启动器无法创建该占用，则记录 `launch_lock_unavailable`，输出稳定错误码 `START_LOCK_UNAVAILABLE` 并以退出码 `2` 结束，不会继续启动后端或客户端。Windows 稳定启动器还使用 Job Object 归属本次启动的后端、客户端及其子进程；启动器正常或异常退出时由系统结束整棵子进程树，不进行宽泛进程清理。若无法建立该归属，则记录 `launch_job_unavailable`，输出稳定错误码 `START_JOB_UNAVAILABLE` 并以退出码 `2` 结束。
...
当前版本支持用户手动下载 `.oliviapatch` 并在本地设置页选择、校验和应用；不需要重新下载完整运行包。公开契约及示例见：

普通用户在客户端 Settings 的“补丁更新”中点击“选择补丁并更新”，选择发布的更新 `.zip` 即可离线校验并安装，无需解压、填写校验码或再次点击安装；成功后关闭并重新打开 Olivia。该页面也提供回滚到上一版本的入口。

更新 ZIP 根目录包含且仅包含一个 `.oliviapatch`、同名 `.oliviapatch.sha256` 和 `.oliviapatch.manifest.sha256`。先验证补丁文件摘要，再由原更新器验证清单摘要和全部文件。内置摘要用于传输完整性检查，不代表发布者数字签名；应从项目发布渠道获取整个 ZIP。构建命令同时生成原补丁、摘要文件和同名 ZIP。已安装的旧版不支持此格式，首次升级需用旧补丁入口或新版完整安装器。

选择旧格式 `.oliviapatch` 时，自动校验只从 `Ornn8/bside-olivia-community` 的对应 GitHub Release 获取 `.oliviapatch.manifest.sha256`（Manifest SHA-256），不访问补丁指定的网址。无法联网或未找到摘要时不安装，已选文件仍保留，可重试或展开“手动校验”粘贴发布说明中的摘要安装。
...
`--manifest-sha256` 必须来自维护者随发布说明提供的值。当前补丁仍依赖这项包外校验；签名校验会作为后续增强加入，不再通过删除补丁入口规避。
