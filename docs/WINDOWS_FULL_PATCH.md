...
普通用户在客户端 Settings 的“补丁更新”中点击“选择补丁并更新”，选择已下载的 `.oliviapatch` 后自动校验并安装，无需填写校验码或再次点击安装；成功后关闭并重新打开 Olivia。该页面也提供回滚到上一版本的入口。

自动校验只从 `Ornn8/bside-olivia-community` 的对应 GitHub Release 获取 `.oliviapatch.manifest.sha256`（Manifest SHA-256），不信任补丁自带摘要，也不访问补丁指定的网址。无法联网或未找到官方摘要时不安装，已选文件仍保留，可重试或展开“手动校验”粘贴发布说明中的摘要安装。旧版本仍使用原来的手动校验流程，安装含此功能的补丁后才出现新入口。
