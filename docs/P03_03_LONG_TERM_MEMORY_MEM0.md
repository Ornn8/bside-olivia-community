...
- Public states are defined by `contracts/mem0_capability_status.schema.json`; v2 includes remaining download bytes, measured installed bytes, stable managed locations, and the accepted source. The route, authorization, request, response, and stable error surface is frozen separately by `contracts/mem0_capability_api_contract.json` and its schema. Installer calls run outside the aiohttp event loop so hashing, disk checks, and uninstall cannot stall sibling routes.
