...
- Program and data volumes receive separate byte budgets before both synchronous and background installation. Progress uses the BOM's runtime/model byte weights, and accepted transport sources persist in owned markers across restarts. Dependencies are byte-compiled once while the UI reports that it is preparing the runtime, then verify before atomic activation under `runtime/mem0-site-packages`; the marker records this compile policy, so a legacy uncompiled runtime is replaced through staging while a failed attempt leaves it intact for retry. An already-ready runtime does not reinstall or recompile on ordinary restart. The model and resumable cache live under `data/memory/model-cache` and `downloads/mem0-model`. Uninstall validates containment, reparse boundaries, and exact ownership markers; failures report `repair`, confirmed model removal also clears its transport cache, and personal memory under `data/memory/mem0` is preserved.
...
- Public states are defined by `contracts/mem0_capability_status.schema.json`; v2 includes remaining download bytes, measured installed bytes, root-qualified managed locations, and the accepted source. The route, authorization, serialized execution, request, response, and stable error surface is frozen separately by `contracts/mem0_capability_api_contract.json` and its schema. Installer calls run outside the aiohttp event loop and serialize per installer, so hashing, disk checks, and uninstall cannot stall sibling routes or race another lifecycle command.
- After a successful original-client sign-in, first-run Settings offers DPAPI-protected LLM configuration and explicit online Mem0 installation, including when sign-in completes while Settings is already open. The same controls remain available in Settings; capability mutations require the login session. The UI explains that skipping LLM setup disables real conversation and points back to Settings. Offline import is visibly unavailable until authenticated external release metadata, Windows archive confinement, and redistribution notices share this online BOM; no offline-package route is exposed and a package cannot be its own trust root.
