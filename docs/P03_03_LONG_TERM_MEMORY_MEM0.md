...
- Dependencies stage and verify before atomic activation under `runtime/mem0-site-packages`; the model and resumable cache live under `data/memory/model-cache` and `downloads/mem0-model`. Uninstall failures report `repair`, confirmed model removal also clears its transport cache, and personal memory under `data/memory/mem0` is preserved.
