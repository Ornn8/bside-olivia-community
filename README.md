...
安装器包含固定版本的核心 Python 运行环境和 **FFmpeg**。语音、翻唱、口型及长期记忆等可选组件仍需在初始设置或本地能力设置中按需安装、导入对应离线包。已有兼容组件可以继续复用，普通补丁升级无需重新下载模型。
...
模型与第三方组件通过薄适配器接入，不包含在源码仓库中。媒体生成失败不能删除已发布正文，也不能因重试重复提交关系变化。历史文档中的 MiniMax、RoFormer、SoulX 方案和固定 60 秒视频描述属于旧链路，当前用法以本页及最新发行说明为准。

## 验证范围

1.1.0 安装器已通过实际隔离安装，内置 Python 和 FFmpeg 可运行，补丁也通过隔离应用；GitHub 对应检查通过。这些结果不代表所有设备都已经验证，遇到安装失败仍需根据日志定位。
