...
def test_internal_spoken_segment_and_complete_musical_renderers(
    ...
    from voice_direction import TextOnlyVoicePlan
    plan = TextOnlyVoicePlan(reply_text)
    scene = tmp_path / "scene.mp4"
    scene.write_bytes(b"scene")
    ...
    assert all("voice_performance_plan" not in letter for letter in letters)
    assert {
...
@pytest.mark.parametrize("helper", ["_voice_plan_for_letter", "_music_voice_plan_for_letter"])
@pytest.mark.parametrize("stored", [None, "corrupt", {"reply_text": "stale", "short_instruction": "whisper"}])
def test_speech_uses_only_frozen_text_without_director_or_cached_controls(monkeypatch, helper, stored):
    text = "今天收到你的信。你想说什么，我都在听。"
    letter = {"letter_id": "text-only"}
    if stored is not None:
        letter["voice_direction_request_id"] = "stale"
    if stored is not None:
        letter["voice_performance_plan"] = stored
    original = dict(letter)

    def forbidden(*args, **kwargs):
        raise AssertionError("text-only speech must not call a director or persist a plan")

    monkeypatch.setattr(local_server, "direct_voice_performance", forbidden, raising=False)
    monkeypatch.setattr(local_server, "_persist_media_state", forbidden)
    build = getattr(local_server, helper)
    plan = asyncio.run(build(letter, text))
    assert asyncio.run(build(letter, text)) == plan
    assert plan.spoken_text == text
    assert plan.to_dict() == {"reply_text": text}
    unit, = plan.speech_units()
    assert (unit.text, unit.speed, unit.pause_after_seconds, unit.gain_db) == (text, 1.0, 0.0, 0.0)
