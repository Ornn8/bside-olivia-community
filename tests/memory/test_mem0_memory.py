...
def test_history_reimport_matches_content_across_file_changes_and_restart(tmp_path, variant):
    ...
    assert write(Mem0ConversationMemoryAdapter(backend, config), old).status is MemoryWriteStatus.WRITTEN
