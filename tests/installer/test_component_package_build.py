...
def test_media_tool_is_in_update_manifest_and_installed(tmp_path, monkeypatch):
    ...
    def stage(payload):
        target = payload / 'media-tools/ffmpeg.exe'
        target.parent.mkdir()
        target.write_bytes(b'fixture-ffmpeg')
    ...
    with zipfile.ZipFile(package) as archive:
        manifest = json.loads(archive.read('manifest.json'))
        assert any(x['path'] == 'media-tools/ffmpeg.exe' for x in manifest['files'])
    ...
    assert len(files) == 1 and files[0].read_bytes() == b'fixture-ffmpeg'
