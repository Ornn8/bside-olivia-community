...
def test_original_settings_management_ui_has_fixed_bounded_contract() -> None:
    assert SETTINGS_UI_VERSION == "p03.original-settings-manage.v37"
    for declaration in (
...
const statusPayload = (status) => ({
  ...
  if (endpoint.pathname === "/toy/cloud/action") {
    return {ok:true,json:async()=>({signed_in:false,publications:[],report_preview:[]})};
  }
