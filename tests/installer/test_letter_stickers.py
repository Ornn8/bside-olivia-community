...
def test_patch_preserves_unrelated_members_and_is_idempotent(tmp_path):
    ...
    with zipfile.ZipFile(path) as z:
        ...
        assert sum(n.endswith('.png') for n in z.namelist())==54
...
const cases=['linli-07','linli-25','../../key.txt','linli-55',''];
...
'''
    ...
    assert rows[1][0]=='linli-25'
    assert all(row[0]=='linli-01' for row in rows[2:])


def test_assets_are_exactly_54_real_transparent_pngs():
    import cv2
