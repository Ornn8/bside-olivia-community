...
INSTRUCTION = (
    ...
    'decision_now 是本轮当前北京时间；user_sent_at 是平台记录的用户原始发送时间，可能因为断线重连而早于本轮收到时间。'
    'delayed_delivery 为 true 时，这是延迟到达的旧消息，不要假装用户刚刚发来，也不要把旧消息里的时间当成现在。'
    '用户提到早饭、午饭、晚饭、睡觉等不能改变当前钟点。餐次和当前活动按 decision_now 与世界状态判断。'
