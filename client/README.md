> 源码目录已归入 `client/`。以下命令均在本目录执行；[返回仓库首页](../README.md)。
...
| 已有可用的 Olivia | 下载 `Olivia-<版本>.oliviapatch`，在设置的“补丁更新”中点击“选择补丁并更新”，选中后自动联网校验并安装，完成后关闭并重新打开程序。旧版入口仍需填写 `.manifest.sha256` 中的校验值；升级后即可使用新流程。 |
...
**1.2.3 新增 AMD 语音实验支持。** AMD 用户在声音与视频组件入口导入专用 `Olivia-voice-amd-1.2.3.zip`，无需先装 NVIDIA 语音包。要求 Windows 11、ROCm 7.2.1 兼容显卡和驱动，建议 12GB 以上显存；保留 Breeze 模型与 2250 音色。尚未完成 AMD 实机生成与听感验收，保守推理路径可能较慢。翻唱和口型视频仍需 NVIDIA。详见 [1.2.3 更新说明](docs/releases/v1.2.3.md)。
